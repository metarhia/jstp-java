...
//                    JSTPConnection.class.getDeclaredMethod("streamPacketHandler", JSObject.class));
    ...
    public JSTPConnection(AbstractSocket transport, RestorationPolicy restorationPolicy) {
        ...
        if (restorationPolicy == null) restorationPolicy = new SessionRestorationPolicy(this);
    ...
    private boolean restoreSession(long numServerSendPackets, long numServerReceivedPackets) {
        ...
        while (redundantPackets-- > 0) sendQueue.poll();
     ...
     * @param handler optional handler that will be called when response handshake
    ...
    public void handshake(String appName, String sessionID, ManualHandler handler) {
        ...
        if (handler != null) handlers.put(packageCounter, handler);
    ...
    public void handshake(String appName, String username, String password, ManualHandler handler) {
        ...
        JSTPMessage hm;
        if (username == null) hm = new JSTPMessage(packageCounter, HANDSHAKE);
        else hm = new JSTPMessage(packageCounter, HANDSHAKE, username, new JSString(password));
        hm.addProtocolArg(appName);
    ...
    private void send(JSTPMessage message, boolean buffer) {
        if (transport.isConnected() || noConnBufferingPolicy == NoConnBufferingPolicy.BUFFER) {
            ...
            if (buffer) {
                ...
                if (sendQueue.size() >= sendBufferCapacity) sendQueue.poll();
            }
        }

        // for now no buffering means no counting
        if (transport.isConnected()) send(message.getStringRepresantation(), buffer);
    }
    ...
    public void onPacketReceived(JSObject packet) {
        try {
            ...
            if (keys.size() == 0 || !(handshake = keys.get(0).equals(HANDSHAKE)) && !handshakeFinished) {
                ...
                else rejectPacket(packet);
            }
            if (!handshake) numReceivedPackets++;
...
//    }
    ...
    public void close(boolean forced) {
        if (transport != null) transport.close(forced);
        sessionID = null;
        reset();
    }
    ...
    public void setClientMethodNames(String... names) {
        for (String name : names) clientMethodNames.add(name);
    }
    ...
    public void onConnected() {
        ...
        if (restorationPolicy != null) restorationPolicy.onTransportAvailable(appName, sessionID);
        else handshake(appName, null);
    ...
    private void reportConnected(boolean restored) {
        for (JSTPConnectionListener listener : connectionListeners) listener.onConnected(restored);
    }

    private void reportError(int errorCode) {
        for (JSTPConnectionListener listener : connectionListeners) listener.onConnectionError(errorCode);
    }
